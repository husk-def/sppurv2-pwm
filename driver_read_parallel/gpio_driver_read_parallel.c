...
void ClearGpioPin(char pin)
{
    ...
    /* simulate led off */
    printk(KERN_INFO "ClearGpioPin: PIN %c OFF\n", pin);
}
...
int gpio_driver_init(void)
{
    ...
    printk(KERN_INFO "gpio_driver_read_parallel: Inserting driver module\n");

    /* Registering device. */
    result = register_chrdev(0, "gpio_driver_read_parallel", &gpio_driver_fops);
    if (result < 0)
    {
        printk(KERN_INFO "gpio_driver_read_parallel: cannot obtain major number %d\n", gpio_driver_major);
        return result;
    }

    gpio_driver_major = result;
    printk(KERN_INFO "gpio_driver_read_parallel: major number is %d\n", gpio_driver_major);
    ...
    /* Setting pull-ups for inputs */
    SetInternalPullUpDown(GPIO_05, PULL_DOWN);
    SetInternalPullUpDown(GPIO_06, PULL_DOWN);
    SetInternalPullUpDown(GPIO_13, PULL_DOWN);
    SetInternalPullUpDown(GPIO_19, PULL_DOWN);
    SetInternalPullUpDown(GPIO_26, PULL_DOWN);
    /* Input */
    SetGpioPinDirection(GPIO_05, GPIO_DIRECTION_IN);
...
fail_no_mem:
    ...
    unregister_chrdev(gpio_driver_major, "gpio_driver_read_parallel");
...
void gpio_driver_exit(void)
{
    printk(KERN_INFO "gpio_driver_read_parallel: Removing driver module\n");

    /* Disable pull-ups */
    SetInternalPullUpDown(GPIO_05, PULL_NONE);
    SetInternalPullUpDown(GPIO_06, PULL_NONE);
    SetInternalPullUpDown(GPIO_13, PULL_NONE);
    SetInternalPullUpDown(GPIO_19, PULL_NONE);
...
{
    /* control variable for sprintf */
    int success_sprintf;
    /* Size of valid data in gpio_driver - data to send in user space. */
    int data_size = 0;

    /* fill gpio_driver_buffer here. */
    success_sprintf = sprintf(gpio_driver_buffer,
    "%d%d%d%d %d",
    GetGpioPinValue(GPIO_26),
    GetGpioPinValue(GPIO_19),
    GetGpioPinValue(GPIO_13),
    GetGpioPinValue(GPIO_06),
    GetGpioPinValue(GPIO_05));

    if (success_sprintf < 0) {
        printk(KERN_INFO "gpio_driver_read_parallel: read - error in sprintf");
    }
    if (*f_pos == 0) {
        /* Get size of valid data. */
        data_size = strlen(gpio_driver_buffer);
        /* Send data to user space. */
        if (copy_to_user(buf, gpio_driver_buffer, success_sprintf + 1) != 0) {
            return -EFAULT;
        } else {
            (*f_pos) += data_size;
...
static ssize_t gpio_driver_write(struct file *filp, const char *buf, size_t len, loff_t *f_pos)
{
    ...
    if (copy_from_user(gpio_driver_buffer, buf, len) != 0) {
        ...
        /* use gpio_driver_buffer here. */
        return strlen(gpio_driver_buffer);
    }
}
